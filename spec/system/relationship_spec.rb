...
RSpec.describe "Relationships", type: :system do
  ...
  let!(:cosme2) { create(:cosme, user: user2) }
  let!(:cosme3) { create(:cosme, user: user3) }
  ...
  describe "フォロー中(following一覧)ページ" do
    ...
    context "ページレイアウト" do
      ...
      it "ユーザー情報が表示されていること" do
        ...
        expect(page).to have_content "コスメ#{user.cosmes.count}件"
  ...
  describe "フィード" do
    ...
    it "フィードに自分の投稿が含まれていること" do
      expect(user.feed).to include cosme
    end

    it "フィードにフォロー中ユーザーの投稿が含まれていること" do
      expect(user.feed).to include cosme2
    end

    it "フィードにフォローしていないユーザーの投稿が含まれていないこと" do
      expect(user.feed).not_to include cosme3
    end
  end
end
